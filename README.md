...
- 🎯 Entry points to execute your program using `python -m <action>` or `$ action` with basic CLI argument parsing.
...
---
# action Flask Application

Awesome action created by xiaoleixiangshuijiao
...
```bash
git clone https://github.com/xiaoleixiangshuijiao/action action
cd action
make install
```
...
```bash
action create-db   # run once
action populate-db  # run once (optional)
action add-user -u admin -p 1234  # ads a user
action run
```
