...
original_urlname="action"
original_description="Awesome action created by xiaoleixiangshuijiao" 
...
then
    ...
    echo "then run 'action' or 'python -m action'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'action' or 'python -m action' or 'poetry run action'"
fi
